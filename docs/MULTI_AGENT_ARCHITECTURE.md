...
所有 Agent 共享同一份代码（`/root/can-dash`），通过 kanban 通信。
...
> **动态生成**：本节由 `hermes kanban assignees` 输出实时填充，避免文档与代码漂移。
> 更新方法：`hermes kanban assignees > /tmp/status.txt` 后粘贴，或在 CI 中自动注入。

### 9.1 任务分配概览（最近一次快照：见 git log）

| Profile | 状态 |
|---------|------|
| backend-agent | 实际看板状态见 `hermes kanban assignees` |
| config-agent | 实际看板状态见 `hermes kanban assignees` |
| default | 协调者，本会话 |
| requirements-agent | 实际看板状态见 `hermes kanban assignees` |
| ui-agent | 实际看板状态见 `hermes kanban assignees` |
| architect-agent | 实际看板状态见 `hermes kanban assignees` |
| release-agent | 实际看板状态见 `hermes kanban assignees` |

### 9.2 Cron 任务

| 任务 | 频率 | 作用 |
|------|------|------|
| `kanban-dispatch-upgrade` | 30m | 调用 `hermes kanban dispatch`，spawn ready 任务 worker |
| `release-check` | 2h | 检查发布时机（无 running/todo + 工作区干净 + 距上次 commit ≥ 3 天） |

**获取当前列表**：`hermes cron list`（带 `last_status` 和 `last_delivery_error`）
